...
pub trait AppExtension: Send + Sync {
    ...
    fn menu_items(&self) -> Vec<(&'static str, fn(&mut redawApp))> {
        Vec::new()
    }
    fn panels(&self) -> Vec<fn(&mut egui::Ui, &mut redawApp)> {
        Vec::new()
    }
    fn on_ui_update(&self, _update: &UIUpdate, _app: &mut redawApp) {}
    fn on_command(&self, _cmd: &AudioCommand, _app: &mut redawApp) {}
}
